...
def build_tensor(eeg_list, pca_list, y_list, sub_list, sex_list):
    """Stack EEG, PCA, and a constant sex channel into a single 3D tensor.

    Args:
        eeg_list: list of ndarrays, each (14, TARGET_EPOCH_SAMPLES).
        pca_list: list of ndarrays, each (4, TARGET_EPOCH_SAMPLES).
        y_list: list of TargetCODE strings.
        sub_list: list of SubjectID strings.
        sex_list: list of 'M'/'F' strings.

    Returns:
        Tuple (X, y, subjects):
        - X: ndarray (n_epochs, 19, TARGET_EPOCH_SAMPLES), float64. Channel
          order is 14 EEG, 4 PCA, then 1 sex channel (constant row of 1.0
          if sex == 'F' else 0.0).
    ...
    for j in range(len(eeg_list)):
        sex_val = 1.0 if sex_list[j] == 'F' else 0.0
    ...
    print(f"Tensor shape : {X_final.shape}  [Epochs x Channels x Time]")
